      ...
      <div className="flex-1 min-h-0 relative">
        {hasImage ? (
          <>
            ...
            {/* Step 2 hint — "add reference" */}
            {step === 2 && (
              <button onClick={() => refInputRef.current?.click()}
                className="absolute top-3 left-1/2 -translate-x-1/2 z-10 bg-black/50 backdrop-blur-md px-4 py-1.5 rounded-full flex items-center gap-2 hover:bg-black/60 transition-colors">
                <IconPlus size={10} className="text-white/60" />
                <span className="text-[10px] text-white/70 tracking-[1px]">Add reference image</span>
              </button>
            )}
            ...
            {step === 3 && (
              <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 bg-black/50 backdrop-blur-md px-4 py-1.5 rounded-full">
                <span className="text-[10px] text-white/50 tracking-[1px]">Tap Apply below to transfer colors</span>
              </div>
            )}
